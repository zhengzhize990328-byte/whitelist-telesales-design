...
(async () => {
  ...
  if ((await app.page.locator('[data-phone="7"] [data-testid="app-history-call"]').count()) !== 3) throw new Error("Angola App daily history does not expose redial for every customer");
  await app.page.locator('[data-phone="7"] [data-testid="app-history-call"]').nth(1).click();
  if (!((await app.page.locator('[data-phone="7"] .call-hero').textContent()).includes("Maria Sebastião"))) throw new Error("Angola App history redial opened the wrong customer");
  ...
  if ((await appZh.page.locator(".device-slot").count()) !== 16) throw new Error("Chinese App explanation does not contain 16 screens");
  if (!((await appZh.page.locator('[data-phone="7"] .screen-title').textContent()).includes("今日全部"))) throw new Error("Chinese App daily history screen is missing");
  if ((await appZh.page.locator('[data-phone="7"] [data-testid="app-history-call"]').count()) !== 3) throw new Error("Chinese App daily history redial list is incomplete");
  await appZh.page.locator('[data-phone="7"] [data-testid="app-history-call"]').nth(2).click();
  if (!((await appZh.page.locator('[data-phone="7"] .call-hero').textContent()).includes("António Domingos"))) throw new Error("Chinese App history redial opened the wrong customer");
  ...
  if (!(await web.page.locator(".follow-center").isVisible())) throw new Error("Bangladesh follow-up center is missing");
  await web.page.locator('[data-follow-tab="history"]').click();
  if ((await web.page.locator(".history-item").count()) !== 3) throw new Error("Bangladesh contact history list is incomplete");
  if ((await web.page.locator("[data-history-call]").count()) !== 3) throw new Error("Every Bangladesh daily-history customer must be callable");
  await web.page.locator('[data-history-call="followup"]').click();
  if (!(await web.page.locator("#callButton").isVisible())) throw new Error("Bangladesh history redial did not open the call page");
  if (!((await web.page.locator(".call-box").textContent()).includes("মারজিয়া"))) throw new Error("Bangladesh history redial opened the wrong customer");
  await web.page.locator('[data-go="today"]').click();
  await web.page.locator('[data-follow-tab="pending"]').click();
  await web.page.locator("[data-follow-action]").click();
  if (!((await web.page.locator(".head h1").textContent()).includes("দ্বিতীয়"))) throw new Error("Bangladesh second-contact task did not open");
  await web.page.locator('[data-go="today"]').click();
  ...
  await web.page.locator("#callButton").click();
  await web.page.locator('[data-result="noanswer"]').click();
  ...
  if (!((await indonesia.page.locator(".follow-center").textContent()).includes("Pusat tindak lanjut"))) throw new Error("Indonesia follow-up center localization is missing");
  if (!((await indonesia.page.locator(".follow-center").textContent()).includes("+62"))) throw new Error("Indonesia follow-up phone localization is missing");
  await indonesia.page.locator('[data-follow-tab="history"]').click();
  if (!(await indonesia.page.locator(".history-item").first().isVisible())) throw new Error("Indonesia contact history tab did not open");
  if ((await indonesia.page.locator("[data-history-call]").count()) !== 3) throw new Error("Every Indonesia daily-history customer must be callable");
  if (!((await indonesia.page.locator('[data-history-call="budi"]').textContent()).includes("Hubungi lagi"))) throw new Error("Indonesia history redial localization is missing");
  await indonesia.page.locator('[data-history-call="budi"]').click();
  if (!((await indonesia.page.locator(".call-box").textContent()).includes("Budi Santoso"))) throw new Error("Indonesia history redial opened the wrong customer");
  if (!((await indonesia.page.locator(".call-box").textContent()).includes("+62 811"))) throw new Error("Indonesia history redial phone localization is missing");
  await indonesia.page.locator('[data-go="today"]').click();
  await indonesia.page.locator('[data-follow-tab="pending"]').click();
  ...
  await indonesiaZh.page.locator('[data-follow-tab="history"]').click();
  if ((await indonesiaZh.page.locator("[data-history-call]").count()) !== 3) throw new Error("Indonesia Chinese daily-history redial list is incomplete");
